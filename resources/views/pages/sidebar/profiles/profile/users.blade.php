...
@section('content')
        @php
            ...
            $accountStatus = $user->account->account_status;
            $isDeactivated = $accountStatus === 'Deactivated';
            $actionRoute = $deactivateRoute;
            $buttonText = $isDeactivated ? 'Reactivate Account' : 'Deactivate Account';
            $buttonClass = $isDeactivated ? 'btn-success' : 'btn-danger';
            $confirmMessage = 'Are you sure you want to ' . ($isDeactivated ? 'reactivate' : 'deactivate') . ' this account?';
        @endphp
            ...
            <div class="profile-container d-flex">
                ...
                <form method="POST" action="{{ $updateRoute }}" enctype="multipart/form-data" class="profile-right-section">
                    ...
                    <div class="profile-grid-container">
                        ...
                        <div class="form-group button-container">
                            <button type="button" class="btn {{ $buttonClass }}" id="deactivateAccountBtn" onclick="if(confirm('{{ $confirmMessage }}')) { document.getElementById('deactivateForm').submit(); }">
                                {{ $buttonText }}
                            </button>
                        </div>
                        <div class="form-group button-container">
        ...
        <form id="deactivateForm" action="{{ $actionRoute }}" method="POST" style="display: none;">
            @csrf
            @method('PUT')
        </form>
