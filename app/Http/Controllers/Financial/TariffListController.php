...
class TariffListController extends Controller
{
    ...
    public function getSuggestedEffectivityDate()
    {
        $currentDateTime = Carbon::now();
        
        // Find the latest tariff list (either active or with the most recent effectivity date)
        $latestTariff = TariffList::orderBy('effectivity_date', 'desc')->first();
        
        if (!$latestTariff) {
            // If no tariffs exist, suggest tomorrow
            $suggestedDate = $currentDateTime->copy()->addDay()->startOfDay();
        } else {
            $latestEffectivityDate = Carbon::parse($latestTariff->effectivity_date)->startOfDay();
            
            // If the latest tariff's effectivity date is in the past or today, suggest tomorrow
            if ($latestEffectivityDate->lte($currentDateTime->copy()->startOfDay())) {
                $suggestedDate = $currentDateTime->copy()->addDay()->startOfDay();
            } else {
                // Otherwise, suggest the day after the latest scheduled tariff
                $suggestedDate = $latestEffectivityDate->copy()->addDay();
            }
        }
        
        return response()->json([
            'suggested_date' => $suggestedDate->format('Y-m-d'),
            'message' => 'This date is calculated as the day after the current/latest tariff becomes inactive.'
        ], 200);
    }
    ...
    protected function updateAllTariffStatuses()
    {
        ...
        foreach ($allTariffLists as $tariffList) {
            $previousStatuses[$tariffList->tariff_list_id] = $tariffList->tl_status;
        }
        ...
        // After updating all statuses, check for Active -> Inactive transitions
        $this->migrateServicesFromInactiveTariffs($allTariffLists, $previousStatuses);
    }
    
    protected function migrateServicesFromInactiveTariffs($allTariffLists, $previousStatuses)
    {
        $currentDateTime = Carbon::now();
        
        foreach ($allTariffLists as $tariffList) {
            $previousStatus = $previousStatuses[$tariffList->tariff_list_id] ?? null;
            $currentStatus = $tariffList->tl_status;
            
            // Check if tariff list transitioned from Active to Inactive
            if ($previousStatus === 'Active' && $currentStatus === 'Inactive') {
                // Get services from this inactive tariff list
                $inactiveServiceIds = ExpenseRange::where('tariff_list_id', $tariffList->tariff_list_id)
                    ->distinct()
                    ->pluck('service_id')
                    ->toArray();
                
                if (empty($inactiveServiceIds)) {
                    continue;
                }
                
                // Find the next scheduled tariff list (with future effectivity date and valid ranges)
                $nextScheduledTariff = $allTariffLists
                    ->filter(function ($tl) use ($currentDateTime) {
                        $effDate = Carbon::parse($tl->effectivity_date)->startOfDay();
                        return $effDate->gt($currentDateTime->copy()->startOfDay()) && $tl->tl_status === 'Scheduled';
                    })
                    ->sortBy(function ($tl) {
                        return Carbon::parse($tl->effectivity_date)->timestamp;
                    })
                ...
                if (!$nextScheduledTariff) {
                    continue;
                }
                
                // Migrate services to the next scheduled tariff list
                foreach ($inactiveServiceIds as $serviceId) {
                    // Check if service already exists in the next scheduled tariff
                    $exists = ExpenseRange::where('tariff_list_id', $nextScheduledTariff->tariff_list_id)
                        ->where('service_id', $serviceId)
                        ->exists();
                    
                    if (!$exists) {
                        // Copy the service with its expense ranges from inactive tariff
                        $existingRanges = ExpenseRange::where('tariff_list_id', $tariffList->tariff_list_id)
                            ->where('service_id', $serviceId)
                            ->get();
                        ...
                        foreach ($existingRanges as $range) {
                            ExpenseRange::create([
                                'exp_range_id' => $this->expenseRangeController->generateExpRangeId(),
                                'tariff_list_id' => $nextScheduledTariff->tariff_list_id,
                                'service_id' => $serviceId,
                        ...
                        Log::info('Migrated service from inactive to scheduled tariff', [
                            'service_id' => $serviceId,
                            'from_tariff' => $tariffList->tariff_list_id,
                            'to_tariff' => $nextScheduledTariff->tariff_list_id,
                        ]);
                    }
                }
            }
    ...
    public function addService(Request $request, string $tariffListId)
    {
        try {
            ...
            if ($exists) {
                return response()->json(['message' => 'This service is already in the tariff list.'], 422);
            }

            // Add the service with null ranges
            ExpenseRange::create([
                'exp_range_id' => $this->expenseRangeController->generateExpRangeId(),
                'tariff_list_id' => $tariffListId,
    ...
    public function removeService(Request $request, string $tariffListId)
    {
        try {
            ...
            // Count how many services are in this tariff list
            $serviceCount = ExpenseRange::where('tariff_list_id', $tariffListId)
                ->distinct('service_id')
                ->count('service_id');

            if ($serviceCount <= 1) {
                return response()->json(['message' => 'Cannot remove the last service. A tariff list must have at least one service.'], 422);
            }

            // Remove all expense ranges for this service
