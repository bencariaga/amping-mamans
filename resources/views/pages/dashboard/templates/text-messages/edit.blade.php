...
@section('breadcrumbs')
    ...
    <a href="{{ route('message-templates.edit', $template) }}" class="text-decoration-none text-white">Edit SMS Template</a>
@endsection

@section('content')
    <div class="container" id="sms-template-container">
        <form id="sms-template-form" action="{{ route('message-templates.update', $template) }}" method="POST">
            @csrf
            @method('PUT')

            <input type="hidden" id="initial-msg-text" value="{{ $template->msg_tmp_text }}">
                ...
                <div class="card template-input-card">
                    ...
                    <div class="details-body">
                        <div class="tmp-title mb-4">
                            ...
                            <input type="text" name="msg_tmp_title" id="msg_tmp_title" class="form-control form-control-lg" placeholder="Example: Text Message Template 1" maxlength="30" required value="{{ old('msg_tmp_title', $template->msg_tmp_title) }}">
...
@section('footer')
    @include('components.layouts.footer.edit-sms-tmp')
