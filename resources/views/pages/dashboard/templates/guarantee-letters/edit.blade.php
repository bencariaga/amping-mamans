...
@section('title', 'Edit GL Template')
...
@section('breadcrumbs')
    ...
    <a href="{{ route('gl-templates.edit', $template) }}" class="text-decoration-none text-white">Edit GL Template</a>
@endsection

@section('content')
    <div class="container" id="gl-template-container">
        <form id="gl-template-form" action="{{ route('gl-templates.update', $template) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <input type="hidden" id="initial-gl-content" value="{{ $template->gl_content }}">
            <input type="hidden" id="initial-signers" value="{{ $template->signers }}">
            <input type="hidden" id="initial-signatures" value="{{ $template->signatures }}">
            <input type="hidden" id="old-gl-content" value="{{ old('gl_content', '') }}">
            <input type="hidden" id="old-signers" value="{{ old('signers', '') }}">
            <input type="hidden" id="existing-signature-1" value="{{ explode(',', $template->signatures)[0] ?? '' }}">
            <input type="hidden" id="existing-signature-2" value="{{ explode(',', $template->signatures)[1] ?? '' }}">
            ...
            <div class="card placeholder-bank-card border-0" id="placeholder-bank-card">
                <div class="card-body px-4 pt-4 pb-2">
                    ...
                    <div class="tmp-title mt-4 mb-1 d-flex flex-row">
                        <input type="text" name="gl_tmp_title" id="gl_tmp_title" class="form-control border-black m-0" placeholder="e.g. Weekday GL Template" maxlength="30" required value="{{ old('gl_tmp_title', $template->gl_tmp_title) }}">
                        ...
                        <div class="formatting-toolbar ms-2 mb-2" id="gl-formatting-toolbar">
                            ...
                            <div class="form-check d-flex align-items-center ms-3">
                                <input class="form-check-input my-0" type="checkbox" id="use-program-head" value="1" @if ($template->use_program_head) checked @endif>
...
@section('footer')
    @include('components.layouts.footer.edit-gl-tmp')
