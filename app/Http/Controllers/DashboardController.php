...
class DashboardController extends Controller
{
    public function showDashboard(): View|RedirectResponse
    {
        ...
        $glYearCount = GuaranteeLetter::join('data', 'guarantee_letters.data_id', '=', 'data.data_id')->whereBetween('data.created_at', [$startOfYear, $endOfYear])->where('data.archive_status', 'Unarchived')->count();
        $glTodayCount = GuaranteeLetter::join('data', 'guarantee_letters.data_id', '=', 'data.data_id')->whereBetween('data.created_at', [$startOfToday, $endOfToday])->where('data.archive_status', 'Unarchived')->count();
