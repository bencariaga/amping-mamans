...
class ClientRegistrationController extends Controller
{
    ...
    /**
     * Remove a patient belonging to an applicant (patients table only).
     */
    public function destroyPatient(Request $request, string $applicant, string $patient)
    {
        $p = \App\Models\User\Patient::where('patient_id', $patient)
        ...
        return back()->with('success', 'Patient removed successfully.');
    }

    /**
     * Store patients under an existing applicant.
     */
    public function storePatients(Request $request, string $applicant)
    {
        ...
        DB::transaction(function () use ($validated, $applicant) {
            // Load existing applicant and its client/account
            $existing = \App\Models\User\Applicant::with('client')
                ->where('applicant_id', $applicant)
                ->firstOrFail();

            $clientIdOfApplicant = $existing->client_id;

            foreach ($validated['patients'] as $index => $patientData) {
                // If include_applicant_as_patient and this is the first index, reuse applicant's client id
                $useApplicant = (($validated['include_applicant_as_patient'] ?? false) && (int)$index === 1);

                if ($useApplicant) {
                    $patientClientId = $clientIdOfApplicant;
                } else {
                    // Create a separate Member + Client for the patient
                    ...
                    Member::create([
                        'member_id' => $pMemberId,
                        // Reuse the applicant's account id
                        'account_id' => \App\Models\User\Member::where('member_id', function($q) use ($clientIdOfApplicant){
                            $q->select('member_id')
                              ->from('clients')
                              ->where('client_id', $clientIdOfApplicant)
                              ->limit(1);
                        })->value('account_id'),
                        'member_type' => 'Patient',
        ...
        return redirect()->route('profiles.applicants.list')->with('success', 'Patient(s) have been added to the existing applicant.');
    }
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'subdivision' => ['nullable', 'string', 'max:50'],
            'purok' => ['nullable', 'string', 'max:50'],
            'sitio' => ['nullable', 'string', 'max:50'],
