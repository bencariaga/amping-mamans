        ...
        foreach ($ranges as $index => $range) {
            ...
            if ($validator->fails()) {
                throw ValidationException::withMessages([
                    'ranges' => [$index => $validator->errors()->toArray()],
                ]);
            }
            ...
            if ($minValue >= $maxValue) {
                throw ValidationException::withMessages([
                    'ranges' => [$index => ['exp_range_max' => 'Maximum value must be greater than minimum value.']],
                ]);
            }
            ...
            if (isset($processedRanges[$rangeKey])) {
                throw ValidationException::withMessages([
                    'ranges' => [$index => ['exp_range_min' => 'This exact range already exists for this service.']],
                ]);
            }
            ...
            foreach ($serviceRanges[$serviceId] as $existingRange) {
                if ($existingRange['min'] === $minValue) {
                    throw ValidationException::withMessages([
                        'ranges' => [$index => ['exp_range_min' => 'This minimum value already exists for this service.']],
                    ]);
                }

                if ($existingRange['max'] === $maxValue) {
                    throw ValidationException::withMessages([
                        'ranges' => [$index => ['exp_range_max' => 'This maximum value already exists for this service.']],
                    ]);
                }

                if ($existingRange['min'] === $maxValue) {
                    throw ValidationException::withMessages([
                        'ranges' => [$index => ['exp_range_max' => 'This maximum value conflicts with an existing minimum value.']],
                    ]);
                }

                if ($existingRange['max'] === $minValue) {
                    throw ValidationException::withMessages([
                        'ranges' => [$index => ['exp_range_min' => 'This minimum value conflicts with an existing maximum value.']],
                    ]);
                }
            }
