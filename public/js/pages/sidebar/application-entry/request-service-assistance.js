document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateMessagePreview() {
        if (!messagePreviewText) {
            console.error('messagePreviewText element not found');
            return;
        }
        ...
        const applicantFirstName = applicantFirstNameInput ? applicantFirstNameInput.value.trim() : '';
        const applicantMiddleName = applicantMiddleNameInput ? applicantMiddleNameInput.value.trim() : '';
        const applicantLastName = applicantLastNameInput ? applicantLastNameInput.value.trim() : '';
        const applicantSuffix = applicantSuffixInput ? applicantSuffixInput.value.trim() : '';
        const patientFirstName = patientFirstNameInput ? patientFirstNameInput.value.trim() : '';
        const patientMiddleName = patientMiddleNameInput ? patientMiddleNameInput.value.trim() : '';
        const patientLastName = patientLastNameInput ? patientLastNameInput.value.trim() : '';
        const patientSuffix = patientSuffixInput ? patientSuffixInput.value.trim() : '';
        const serviceType = serviceTypeDropdownBtn ? serviceTypeDropdownBtn.textContent.trim() : '';
        const affiliatePartner = affiliatePartnerDropdownBtn ? affiliatePartnerDropdownBtn.textContent.trim() : '';
        const billedAmount = billedAmountInput ? billedAmountInput.value : '';
        const assistanceAmount = assistanceAmountInput ? assistanceAmountInput.value : '';
        const appliedAt = dateAppliedInput ? dateAppliedInput.value : '';
        const reapplyAt = dateToReapplyInput ? dateToReapplyInput.value : '';
    ...
    async function filterAffiliatePartnersByService(serviceId) {
        console.log('Filtering affiliate partners by service:', serviceId);
        
        if (!serviceId) {
            // Reset to show all affiliate partners
            if (affiliatePartnerDropdownBtn) {
                affiliatePartnerDropdownBtn.textContent = 'Select an affiliate partner.';
            }
            if (affiliatePartnerInput) {
                affiliatePartnerInput.value = '';
            }
            return;
        }

        try {
            const url = `/applications/affiliate-partners-by-service?service_id=${serviceId}`;
            ...
            // Clear current affiliate partner selection
            if (affiliatePartnerDropdownBtn) {
                affiliatePartnerDropdownBtn.textContent = 'Select an affiliate partner.';
            }
            if (affiliatePartnerInput) {
            ...
            // Update the affiliate partner dropdown with filtered results
            const affiliateMenu = affiliatePartnerDropdownBtn ? affiliatePartnerDropdownBtn.nextElementSibling : null;
            
            if (!affiliateMenu) {
                console.error('Affiliate partner dropdown menu not found');
                return;
            }
            
            affiliateMenu.innerHTML = '';
            
            // Add default option
            const defaultItem = document.createElement('li');
            defaultItem.innerHTML = '<a class="dropdown-item" href="#" data-value="">Select an affiliate partner.</a>';
            affiliateMenu.appendChild(defaultItem);
            
            // Add click handler for default option
            defaultItem.querySelector('a').addEventListener('click', function (event) {
                event.preventDefault();
                if (affiliatePartnerDropdownBtn) {
                    affiliatePartnerDropdownBtn.textContent = 'Select an affiliate partner.';
                }
            ...
            // Add filtered affiliate partners
            if (result.affiliate_partners && Array.isArray(result.affiliate_partners) && result.affiliate_partners.length > 0) {
                result.affiliate_partners.forEach(partner => {
                    if (!partner || !partner.affiliate_partner_id || !partner.affiliate_partner_name) {
                    ...
                    link.className = 'dropdown-item';
                    link.href = '#';
                    link.setAttribute('data-value', partner.affiliate_partner_id);
                    link.textContent = partner.affiliate_partner_name;
                    ...
                    item.appendChild(link);
                    affiliateMenu.appendChild(item);
                });
                
                console.log(`Added ${result.affiliate_partners.length} affiliate partners to dropdown`);
            } else {
                const noResultItem = document.createElement('li');
                noResultItem.innerHTML = '<a class="dropdown-item disabled" href="#">No affiliate partners available for this service.</a>';
                affiliateMenu.appendChild(noResultItem);
                console.log('No affiliate partners found for this service');
            }
            
            updateMessagePreview();
        } catch (error) {
            console.error('Error fetching affiliate partners:', error);
            alert('Failed to load affiliate partners. Please try again or refresh the page.');
        }
    }
    ...
    if (serviceMenu) {
        serviceMenu.querySelectorAll('.dropdown-item').forEach(item => {
            item.addEventListener('click', function (event) {
                ...
                // Filter affiliate partners based on selected service
                filterAffiliatePartnersByService(value);
